class CompanyProfile < ActiveRecord::Base
  ...
  validates :company_name, :uniqueness => true,  :presence => { :case_sensitive => false }
  validates_format_of :primary_phone, :with => /^1[35]\d{9}$|^((0\d{2,3})-)(\d{7,8})(-(\d{3,}))?$/, :allow_blank => true
  validates_format_of :secondary_phone, :with => /^1[35]\d{9}$|^((0\d{2,3})-)(\d{7,8})(-(\d{3,}))?$/, :allow_blank => true
